...
public class NDGAct extends AppCompatActivity implements BackHandledInterface {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_httpcapmain);
    ...
    public void createPage() {
        ...
        Snackbar.make(rootView, "HAR file has created a new page:" + time, Snackbar.LENGTH_LONG)
...
//            setSelectedFragment((BaseFragment) to);
                    ...
                    public void run() {
                        try {
                            ...
                            rootView.post(new Runnable() {
                                @Override
                                public void run() {
                                    Snackbar.make(rootView, "HAR file has been saved to " + saveHarFile.getPath() + " Totally there are"
                            ...
                            rootView.post(new Runnable() {
                                @Override
                                public void run() {
                                    Snackbar.make(rootView, "HAR file failed to save", Snackbar.LENGTH_LONG).setAction("Action", null).show();
                                }
                            });
                            e.printStackTrace();
                        } finally {
    ...
    public class MyUploadDelegate implements UploadStatusDelegate {
        ...
        public void onError(Context context, UploadInfo uploadInfo, ServerResponse serverResponse, Exception exception) {
            dismissLoading();
            Snackbar.make(rootView, "Failed to upload!", Snackbar.LENGTH_LONG).setAction("Action", null).show();
        ...
        public void onCompleted(Context context, UploadInfo uploadInfo, ServerResponse serverResponse) {
            try {
                ...
                if (jsonObject.getInt("errId") == 0) {
                    Snackbar.make(rootView, "Succeeded in uploading", Snackbar.LENGTH_LONG).setAction("Action", null).show();
                } else if (jsonObject.getInt("errId") == 2 || jsonObject.getInt("errId") == 11004) {
                    Snackbar.make(rootView, "Verification code error!", Snackbar.LENGTH_LONG).setAction("Action", null).show();
